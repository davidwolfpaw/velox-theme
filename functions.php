		...
		add_theme_support(
			...
			array(
				array(
					...
					'slug'  => 'text',
					'color' => get_theme_mod( 'text_color', '#1d2327' ),
				),
				array(

					'name'  => __( 'Accent Text Color', 'velox' ),
					'slug'  => 'accent-text',
					'color' => get_theme_mod( 'accent_text_color', '#50575e' ),
				),
				array(
...
function velox_editor_styles() {
	// Editor styles.
	wp_enqueue_style( 'velox-editor-style', get_template_directory_uri() . '/css/editor-style.css', '1.2.1', 'all' );
}
...
function velox_scripts() {
	// Primary Styles.
	wp_enqueue_style( 'velox-style', get_stylesheet_uri(), array(), '1.2.1', 'all' );
	...
	wp_enqueue_script( 'velox-scripts', get_template_directory_uri() . '/js/velox-scripts.js', array( 'wp-i18n' ), '1.2.1', true );
	...
	$velox_options = array(
		...
		'link_color'         => sanitize_hex_color_no_hash( get_theme_mod( 'link_color', '0a4b78' ) ),
	);
